...
# Modified:			2023.12.14	针对补测数据

############################################################


my $merged_fq_buce = shift;
...
die  "
Usage:
perl  xxx.pl  merged_fq_buce  primer_table  genotype_table  barcode_row  barcode_col  out
...
my($col_tag, $insert_seq, $row_tag, $tag);
my(%indel_forward2tag);
my($number, $threshold1, $top1, $top2);
############################################################
...
while (<IN>) {
	...
	if($t[0] =~ /[Ii]n[Dd]el_?(\d+)/){
		...
		$indel_forward2tag{$t[2]} = $indel_tag;
	}
}
close IN;
	...
	#$_ =~ s/[\r\n]+//g;
		...
		while (<IN2>){
			...
			if($i == 1){
				...
				foreach $indel_tag (sort keys %indel_group){
					...
					if($t[$i] =~ /([ATCG]{5})$forward{$indel_tag}(\w+)$reverse_rc{$indel_tag}([ATCG]{5})/){			
						...
						${"total_".$indel_tag}{$row_tag."_".$col_tag} ++;
						${"count_".$indel_tag."_".$row_tag."_".$col_tag}{$insert_seq} ++;
	...
	foreach $m(sort keys %barcode_row_group){
		foreach $n(sort {$a <=> $b} keys %barcode_col_group){
			...
			foreach $i (sort {$a <=> $b} keys %indel_group){
				...
				#if (${"total_".$i}{$j."_".$x."_".$y} >= 1000){
				if (${"total_".$i}{$x."_".$y} && ${"total_".$i}{$x."_".$y} >= 1000){	#每个孔的reads总数 >= 1000;
					undef $tag;
					undef $number; #循环数, 只记录Top1, Top2;
					undef $top1;
					undef $top2;
					$threshold1 = ${"total_".$i}{$x."_".$y} *0.3;
					
					#foreach $z ( sort {${"count_".$i."_".$j."_".$x."_".$y}{$b} <=> ${"count_".$i."_".$j."_".$x."_".$y}{$a}} keys %{"count_".$i."_".$j."_".$x."_".$y} ){
					foreach $z ( sort {${"count_".$i."_".$x."_".$y}{$b} <=> ${"count_".$i."_".$x."_".$y}{$a}} keys %{"count_".$i."_".$x."_".$y} ){
						$number ++;
						
						if($number == 1){
							$top1 = ${"count_".$i."_".$x."_".$y}{$z};
